...
$('.slide').first().fadeTo(0, 1).addClass('current');
...
$('.slide-prev').text('&').css('font','20px IconicStrokeRegular').click(function(){
	...
	if ( current_position >= 2 ) {
		...
		--current_position;
		$('.slide:nth-child(' + current_position + ')').fadeTo(500, 1).addClass('current');
		
		// Make sure the last slide is not current
		var previous_position = current_position + 1;
		$('.slide:nth-child(' + previous_position + ')').fadeTo(300, 0.3).removeClass('current');
